...
import { Plus, Pencil, Trash2, Save, X, ArrowLeft, Code2, Loader2 } from 'lucide-react'
import FileUploader from '@/components/boss/FileUploader'
import type { ProgrammingProject } from '@/types/content'

const EMPTY: Omit<ProgrammingProject,'id'> = { category:'programming', title:'', description:'', tags:[], date:'', featured:false, status:'completed', tech_stack:[], github_url:'', live_url:'' }

export default function BossProgrammingPage() {
  const { clientId } = useParams() as { clientId: string }
  const [projects, setProjects] = useState<ProgrammingProject[]>([])
  const [editing, setEditing] = useState<ProgrammingProject | null>(null)
  ...
  useEffect(() => {
    fetch(`/api/github?clientId=${clientId}&path=projects/programming.json`)
      .then(r => r.json()).then(d => { setProjects(d||[]); setLoading(false) }).catch(() => setLoading(false))
  }, [clientId])

  const startNew = () => { setEditing({ ...EMPTY, id:`prog_${Date.now()}` } as ProgrammingProject); setIsNew(true) }
  ...
  const save = async () => {
    ...
    const res = await fetch('/api/github', { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ clientId, path:'projects/programming.json', content: JSON.stringify(updated,null,2) }) })
  ...
  const remove = async (id:string) => {
    ...
    await fetch('/api/github', { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ clientId, path:'projects/programming.json', content: JSON.stringify(updated,null,2) }) })
    setProjects(updated)
  }
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-0)] p-4 max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="w-8 h-8 rounded-xl bg-emerald-400/10 flex items-center justify-center"><Code2 className="w-4 h-4 text-emerald-400" /></div>
        <h1 className="text-xl font-bold text-[var(--t1)]">Programming Projects</h1>
      ...
      {editing && (
        <div className="card p-6 mb-6 border-folyx-500/30">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-semibold text-[var(--t1)]">{isNew ? 'New' : 'Edit'} Programming Project</h2>
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-4">
            <div><label className="block text-xs text-[var(--t3)] mb-1">Tech Stack</label>
              <input value={editing.tech_stack?.join(', ')} onChange={e=>set('tech_stack',e.target.value.split(',').map((t:string)=>t.trim()).filter(Boolean))} className="input-folyx" placeholder="Python, Django, React" /></div>
            <div><label className="block text-xs text-[var(--t3)] mb-1">Tags</label>
              <input value={editing.tags?.join(', ')} onChange={e=>set('tags',e.target.value.split(',').map((t:string)=>t.trim()).filter(Boolean))} className="input-folyx" placeholder="AI, Web, CLI" /></div>
          </div>
          <div className="grid sm:grid-cols-2 gap-4 mb-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-4">
            ...
            <div className="flex items-end pb-1">
              <label className="flex items-center gap-2 text-sm text-[var(--t2)] cursor-pointer">
                <input type="checkbox" checked={editing.featured} onChange={e=>set('featured',e.target.checked)} className="w-4 h-4 rounded" />Featured
              </label>
            </div>
          </div>
      ...
      : projects.length===0 ? <div className="card p-12 text-center text-[var(--t3)]">No programming projects yet.</div>
      : <div className="space-y-3">{projects.map(p=>(
          <div key={p.id} className="card p-4 flex items-center justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-[var(--t3)]">{p.tech_stack?.slice(0,3).join(' · ')}</p>
            </div>
