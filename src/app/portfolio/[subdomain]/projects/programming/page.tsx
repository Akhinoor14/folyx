...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `Programming — ${c?.personal.full_name || ''}` }
}

export default async function ProgrammingPage({ params }: Props) {
  const subdomain = headers().get('x-folyx-subdomain') || params.subdomain
  const client = await getClientInfo(subdomain)
  if (!client) notFound()

  const raw = await getGitHubFile(`${subdomain}-portfolio`, 'projects/programming.json')
  const projects: ProgrammingProject[] = raw ? JSON.parse(raw) : []
    ...
    <div className="pt-24">
      <section className="section-sm">
        <div className="container-folyx">
          ...
          <h1 className="mb-2">Programming Projects</h1>
          <p className="text-[var(--t2)] mb-10">Software, algorithms, scripts, and more.</p>

          {projects.length === 0 ? (
            <div className="card p-12 text-center text-[var(--t3)]">No programming projects added yet.</div>
          ) : (
                  ...
                  {p.tech_stack?.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-3">
                      {p.tech_stack.slice(0, 5).map(t => (
                        <span key={t} className="text-xs px-2 py-0.5 rounded-md bg-emerald-400/10 text-emerald-400 border border-emerald-400/20">{t}</span>
                      ))}
                    </div>
                  )}
                </ProjectCard>
